...
<script>
    ...
    // ⚡ FUNCIÓN CORREGIDA: ENVIAR COMO JSON (clave para que funcione)
    function enviarReporteConCoordenadas(coords, form, submitBtn, originalText) {
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Enviando...';
        
        // ✅ Extraer datos manualmente y enviar como JSON
        const data = {
            calle_principal: form.querySelector('[name="calle_principal"]').value,
            cruzamiento1: form.querySelector('[name="cruzamiento1"]').value || null,
            cruzamiento2: form.querySelector('[name="cruzamiento2"]').value || null,
            colonia: form.querySelector('[name="colonia"]').value,
            nivel_afectacion: form.querySelector('[name="nivel_afectacion"]').value,
            prioridad: form.querySelector('[name="prioridad"]').value,
            descripcion: form.querySelector('[name="descripcion"]').value || null,
            latitud: coords.lat,
            longitud: coords.lng,
            metodo_origen: 'web_usuario',
            fuente_ubicacion: coords.fuente,
            estado_reporte_id: 1
        };
